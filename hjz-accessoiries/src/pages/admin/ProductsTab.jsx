...
import { IconBox, IconCheckCircle, IconBan, IconLayers, IconPlus, IconTag, IconTrash } from '../../components/AdminIcons.jsx'
import { CATEGORIES, categoryLabel } from '../../data/categories.js'

const MAX_PHOTOS = 5

const emptyForm = { name: '', price: '', category: CATEGORIES[0].id, hasSize: false, sizesText: '', stock: '' }
    ...
    <>
      <div className="admin-stats">
        <div className="admin-stat-card">
          <span className="admin-stat-card__icon"><IconBox /></span>
          <span>
        ...
        <div className="admin-stat-card admin-stat-card--success">
          <span className="admin-stat-card__icon"><IconCheckCircle /></span>
          <span>
        ...
        <div className="admin-stat-card admin-stat-card--danger">
          <span className="admin-stat-card__icon"><IconBan /></span>
          <span>
        ...
        <div className="admin-stat-card admin-stat-card--neutral">
          <span className="admin-stat-card__icon"><IconLayers /></span>
          <span>
      ...
      <form className="admin-card admin-form" onSubmit={submit}>
        <h3><IconPlus /> Ajouter un produit</h3>
        ...
        <div className="field">
          ...
          >
            {CATEGORIES.map((c) => (
              <option key={c.id} value={c.id}>{c.label.fr}</option>
            ))}
          </select>
        </div>
      ...
      <div className="admin-card">
        <h3><IconBox /> Produits ({products.length})</h3>
            ...
            return (
              <div key={p.id} className={`admin-product-row ${Number(p.stock) <= 0 ? 'is-sold-out' : ''}`}>
                ...
                <div className="admin-product-row__info">
                  ...
                  <span className="admin-tag"><IconTag /> {categoryLabel(p.category, 'fr')}</span>
                  {p.has_size && <span className="admin-tag"><IconTag /> Tailles : {p.sizes.join(', ')}</span>}
                  {Number(p.stock) <= 0 && <span className="admin-tag admin-tag--sold"><IconBan /> Épuisé côté client</span>}
                </div>
                ...
                <button className="btn admin-delete" onClick={() => deleteProduct(p.id)}><IconTrash /> Supprimer</button>
              </div>
            )
          })}
        </div>
